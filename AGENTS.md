...
- 若通过测试 harness 批量生成产物，默认跑 `bayer`、`blue-noise`、`atkinson`、`floyd-steinberg`、`clustered-dot`、`yliluoma`，用于自动化批量对比。
...
- 若只调单个算法，优先设置 `EPAPER_HARNESS_MODES=<slug>`，避免批量生成其他模式的对比图。
