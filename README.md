...
| 屏幕尺寸 | `7.3` 寸                                               |
| 分辨率   | `800x480`                                              |
| 像素密度 | 约 `128 PPI`（按 `7.3` 寸对角线估算）                  |
| 调色板   | `Black`、`White`、`Red`、`Yellow`、`Blue`、`Green`     |
| 输出编码 | 每像素 `1` 字节，像素值范围 `0..5`，可直接对接驱动数组 |

**算法调优参考**：

- 该面板为 `7.3` 寸、`800x480`、约 `128 PPI`，属于手持近看时网点会比较明显的档位。
- 后续实现或调优半色调/抖动算法时，需把“网点存在感不要先于主体内容被看到”作为默认约束，尤其关注中间调颗粒感。
- 对 `clustered-dot` 一类成团网点算法，默认应优先选择更克制的参数；当前仓库默认强度为 `38`，可作为后续调参起点。
